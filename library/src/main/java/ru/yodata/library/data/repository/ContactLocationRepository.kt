...
class ContactLocationRepository(
    ...
    override suspend fun getLocatedContactList(): List<LocatedContact>? =
            withContext(Dispatchers.IO) {
                Log.d(TAG, "ContactLocationRepository: начинаю чтение списка контактов из БД...")
                val locatedContactList: MutableList<LocatedContact>? =
                        //withContext(Dispatchers.IO) {
                ...
                Log.d(TAG, "ContactLocationRepository: список контактов из БД прочитан")
                Log.d(TAG, "locatedContactList = ${locatedContactList?.toString()}")
